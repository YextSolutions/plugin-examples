...
const basicAuthEnconded = btoa(`${username}:${password}`);
...
const baseTicketEndpoint = `https://${ZENDESK_SUBDOMAIN}.zendesk.com/api/v2/tickets?page[size]=5`;

/*
 * Fetches a page of tickets and their comments from the Zendesk instance specified by ZENDESK_SUBDOMAIN.
 * If the Zendesk rate limit has been reached, the function sleeps for a period of time and persists
...
export const fetchTickets = async (inputString: string) => {
    ...
    if (ticketsPage.retryAfter) {
        ...
        const ticketArray = ticketsPage.tickets;
        const tickets: any[] = [];
        ...
        const nextTicketPageUrl = ticketsPage.meta.has_more ? ticketsPage.links.next : undefined;
...
const fetchComments = async (ticketId: any) => {
    const commentListEndpoint = `https://${ZENDESK_SUBDOMAIN}.zendesk.com/api/v2/tickets/${ticketId}/comments?page[size]=100`;
    ...
    return { comments: firstPage.comments};
}